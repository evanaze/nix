# llama.cpp 0.2.0 (build 10566, commit bb4caa7) vendored from nixpkgs rev e8be7818.
#
# WHY THIS EXISTS
# --------------
# The nixos-26.05 channel tree pinned by this flake (rev c5c4a43b / "nixos/nixpkgs")
# still ships llama-cpp build b9190 (May 2026), which PREDATES upstream commit
# 7d2b45b4 (2026-06-08, PR #24282) that added MTP "gemma4-assistant" architecture
# support. As a result loading the MTP draft model:
#
#   /mnt/jupiter-llama-models/mtp-gemma-4-12B-it.gguf
#
# fails with "unknown model architecture: 'gemma4-assistant'".
#
# llama.cpp v0.2.0 (build 10566) contains that support. This package is a verbatim
# copy of nixpkgs' llama-cpp package.nix at rev e8be7818, which packages exactly
# that release. Once nixos-26.05 advances past llama-cpp 0.2.0 this override can be
# removed and the module can go back to plain `pkgs.llama-cpp`.
{
  lib,
...
let
  ...
  cudaBuildInputs = with cudaPackages; [
    cuda_cccl # <nv/target> (attr is cuda_cccl in this nixpkgs; "cccl" in newer channels)
